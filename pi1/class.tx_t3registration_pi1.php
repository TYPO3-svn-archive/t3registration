...
class tx_t3registration_pi1 extends tslib_pibase {
    ...
    protected function evaluateField($value,$evaluationRule,$field = array()){
        switch($evaluationRule){
            ...
            case 'file':
                $files = array();
                //@da sostituire con una variabile normale
                $this->fileFields[$field['name']] = $this->piVars[$field['name']];
                $noError = true;
                foreach($_FILES[$this->prefixId]['name'][$field['name']] as $key => $item){
                    if(strlen($item) > 0){
                        ...
                        if($file === true){
                            ...
                            $this->fileFields[$field['name']][] = $file;
                        }
                    }
                }
                $this->piVars[$field['name']] = (is_array($this->fileFields[$field['name']]))?implode(',',$this->fileFields[$field['name']]):$this->fileFields[$field['name']];
                return $noError;
                break;
            case 'hook':
    ...
    public function getAndReplaceSubpartPreview($field,$content,$contentArray){
        if(isset($field['config']['internal_type']) && $field['config']['internal_type'] == 'file'){
            ...
            $contentArray['###' . strtoupper($field['name']) . '_VALUE###'] = (($field['hideInChangeProfile'] == 1 && $GLOBALS['TSFE']->loginUser) || strlen($this->piVars[$field['name']]) == 0)?'':$this->htmlentities($this->piVars[$field['name']]);
        }
        return $contentArray;
    }
    ...
    private function insertUser(){
        ...
        if(!$this->checkUniqueField($user['username'],$this->fieldsData[$this->conf['usernameField']])){
            $this->getForm();
        }
