...
class tx_t3registration_pi1 extends tslib_pibase {
     ...
     * @var array
     */
    private $flexformOverrideTs = array('contactEmailMode','approvalProcess','userFolder', 'templateFile','autoLoginAfterConfirmation');
        ...
        switch ($field['config']['type']){
            ...
            case 'check':
                if(isset($field['config']['items']) && is_array($field['config']['items'])){
                    ...
                    $this->piVars[$field['name']] = ($this->piVars[$field['name']])?$this->piVars[$field['name']]:(($field['config']['default'])?$field['config']['default']:'');
                    $this->piVars[$field['name']] = ($this->piVars[$field['name']])?$this->piVars[$field['name']]:(($field['config']['default'])?$field['config']['default']:'');
                    $text = (preg_match('/LLL:EXT:/',$field['config']['text'])?$this->cObj->stdWrap($GLOBALS['lang']->sl($item[0]),$this->conf['fieldConfiguration.'][$field['name'] . '.']['config.']['text.']['stdWrap.']):$this->cObj->stdWrap($field['config']['text'],$this->conf['fieldConfiguration.'][$field['name'] . '.']['config.']['text.']['stdWrap.']));
                    $value = '1';
